...
class CrossEntropy2d(nn.Module):
    ...
    def forward(self, out, target):
        n, h, w = out.size()         # n:batch_size, c:class
        out = out.view(-1, 1)           # (n*h*w, c)
        target = target.view(-1).cuda()        # (n*h*w)
...
class CustomLoss(nn.Module):
    ...
    def forward(self, preds, targets):
        ...
        ################################################################
        # cls_loss = self.cross_entropy2d(cls_preds, cls_targets)
        ...
        # print('loc_loss: %.8f | cls_loss: %.8f' % (loc_loss.data, cls_loss.data))
        return cls_loss+loc_loss, cls_loss, loc_loss


class MultiTaskLoss(nn.Module):
    r"""Multi-task loss with adaptive weghts

    Formulation:
        loss_{total} = \sum_i (\frac{1}{\sigma_i^2}* loss_i + \frac{1}{2}*\log(\sigma_i^2+1)
    """
    ...
    def __init__(self, num_losses):
        ...
        Example::
            criterion = MultiTaskLoss(3)
            ...
            # add parameters to optimizer
            optimzier = Adam(params=[{'params': criterion.parameters()}])

            # forward
            loss = criterion([loss_1, loss_2, loss_3])

            # backward
            loss.backward()
        Warning:
            you need to save weights of multi-loss mamually for resuming the experiment next time
